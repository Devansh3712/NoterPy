...
while True:
	...
	if nu.User.check(username, password) is True:
	...
	# if user doesn't exist in user.txt
		...
		if user_prompt.lower() == 'y':
			if nu.User.new(username, password) is False:
		...
		# user settings
			...
			while True:
				...
				if user_prompt_list == '1':
					...
					password = getpass.getpass(prompt = "Password: ")
					if nu.User.update(name, new_name, password) is False:
						print('Username already exists, try another name\n')
					else:
				...
				# export logs of a user
					...
					if udb.User.export_logs(name, date) is False:
